...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y, rep=None, drop_rep=True):
        """
        Forward pass of DiT.
        ...
        if rep is not None:
            ...
            if len(rep.shape) == len(t.shape):
                c = t + rep  # (N, D)
            else:
                c = t.unsqueeze(1) + rep  # (N, T, D)
        else:
            c = t + y  # (N, D)
